...
void guardar_emails(){
	...
	for(i=0;i<infos_ut_size;i++){
		for(j=0;j<infos_ut[i].num_emails;j++){
			fprintf(fp,"%d %d %s %s %s %s",i, j, infos_ut[i].utilizador, infos_ut[i].emails[j].remetente, infos_ut[i].emails[j].assunto, infos_ut[i].emails[j].texto);
		}
	}
	fclose(fp);
	exit(0);
...
void process_client(int client_fd)
{
	...
	if(strcmp(buffer,"enviar\n")==0){
		...
		printf("Utilizador: %s\n",utilizador);
		//num_destinatarios
		nread = read(client_fd,cnum_destinatarios,INFO_SIZE-1);
		fflush(stdout);
...
void adicionar_emails_ut(int fd, char destinatario[INFO_SIZE], char utilizador[INFO_SIZE], char assunto[INFO_SIZE], char texto[BUF_SIZE]){
	int i;
	//printf("Entrou na funçao inserir_email; agora vai procurar o destinatario %s \n",destinatario);
	for(i=0;i<infos_ut_size;i++){
		//printf("%s",infos_ut[i].utilizador);
		if(strcmp(infos_ut[i].utilizador,destinatario)==0){
			//printf("Esta a guardar o email na lista\n");
